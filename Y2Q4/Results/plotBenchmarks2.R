...
write.table(data, file=paste0("~/Documents/PhD/Code/Y2Q3/Scripts/Results/",filename,".csv"), col.names=F, row.names=F, sep=',')
data = data[, c(2, 3, 5, 1)]
...
g = ggplot(data = data, aes(x = factor(Dataset), y = Time, group = Implementation, 
                            colour = Implementation, shape = Implementation)) + geom_line(aes(linetype = Implementation)) + 
  geom_point(size = 2) + labs(title = title, y = "Time (sec)") + scale_x_discrete("Number of points", 
                                                                                  breaks = paste0(seq(20, 100, 20), "K")) + ## scale_x_discrete('Number of points') +
  theme(axis.text.x = element_text(size = 8, angle = 90), axis.text.y = element_text(size = 8)) + 
  facet_wrap(~Epsilon) + scale_colour_discrete() + scale_shape_discrete() + scale_linetype_discrete()
filename = paste0(strsplit(filename, "\\.")[[1]][1:length(strsplit(filename, "\\.")[[1]]) - 
                                                   1], collapse = ".")

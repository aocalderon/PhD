 ...
 * <br/><br/>
 * <p>
 * High Performance Mining of Maximal Frequent SPMF.Itemsets (with J. Zhu).
...
public class AlgoFPMax0 {
    ...
    public Itemsets runAlgorithm(String input, String output, double minsupp) throws IOException {
        ...
        // Before inserting a transaction in the SPMF.FPTree, we sort the items
    ...
    public Itemsets runAlgorithm(Set<List<Integer>> input, int minsup) throws IOException {
        // record start time
        startTimestamp = System.currentTimeMillis();
        // number of itemsets found
        itemsetCount = 0;
    ...
    public Itemsets runAlgorithm(Iterator<List<Integer>> input, int minsup) throws IOException {
        // record start time
        startTimestamp = System.currentTimeMillis();
        // number of itemsets found
        itemsetCount = 0;
        ...
        while (input.hasNext()) {
            List<Integer> original = input.next();
            ...
            Collections.sort(filtered, comparatorOriginalOrder);
    ...
    private Map<Integer, Integer> scanDatabaseToDetermineFrequencyOfSingleItems(Set<List<Integer>> input) {
        Map<Integer, Integer> mapSupport = new HashMap<Integer, Integer>();
    ...
    private Map<Integer, Integer> scanDatabaseToDetermineFrequencyOfSingleItems(Iterator<List<Integer>> input) {
        Map<Integer, Integer> mapSupport = new HashMap<Integer, Integer>();


        while (input.hasNext()) {
            List<Integer> transaction = input.next();

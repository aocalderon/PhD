...
/**
  ...
  */
object MySTRPartition {
	var partitioner: STRPartitioner = _

	def apply(origin: RDD[(Point, Long)]
	          , dimension: Int
	          , est_partition: Int
	          , sample_rate: Double
	          , transfer_threshold: Long
	          , max_entries_per_node: Int): (RDD[(Point, Long)], Array[(MBR, Int)]) = {
		val rdd = origin
			.mapPartitions { iter =>
				val mutablePair = new MutablePair[Point, Long]()
				iter.map(row => mutablePair.update(row._1, row._2))
			}

		partitioner = new STRPartitioner(est_partition, sample_rate, dimension,
			transfer_threshold, max_entries_per_node, rdd)
		val shuffled = new ShuffledRDD[Point, Long, Long](rdd, partitioner)
		(shuffled, partitioner.mbrBound)
	}
}

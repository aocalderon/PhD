...
import $ivy.`org.apache.spark::spark-catalyst:2.1.0`
...
val spark = SparkSession.builder().master("spark://169.235.27.138:7077").appName("Tester").config("spark.cores.max", "32").getOrCreate()
...
cross.cache
val mu = 12
val f = cross.map{ r => (r.getString(0), r.getInt(1), r.getString(2), r.getInt(3), r.getString(0).split(";").intersect(r.getString(2).split(";")).length)}.filter(r => r._5 > mu) 
f.cache
f.count
val n = f.count

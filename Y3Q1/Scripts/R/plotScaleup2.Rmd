...
PATH = "Y3Q1/Scripts/Scaleup/Scaleup_2017-10-12/"
...
### Experiments using spatial sampling (Test Number 2).

```{r ScaleupSetup2, include=FALSE}
PATH = "Y3Q1/Scripts/Scaleup/Scaleup_2017-10-13/"
filename ="Berlin_N20K-80K_E50.0-100.0"
PHD_HOME = Sys.getenv(c("PHD_HOME"))
...
### Number of candidate and maximal disks (Test Number 1).
...
### Number of candidate and maximal disks (Test Number 2).

```{r DisksSetup2, include=FALSE}
PATH = "Y3Q1/Scripts/Scaleup/Scaleup_2017-10-13/"
filename ="Berlin_N20K-80K_E50.0-100.0"
files = system(paste0("ls ",PHD_HOME,PATH,filename,"_C*.csv"), intern = T)
data = data.frame()
...
```{r Candidates2, echo=FALSE, fig.width=10.5, fig.height=7.5, fig.align='center'}
candidates = data[data$Disks == "Candidates",]
temp_title = paste("(radius of disk in mts) in Berlin dataset.")
title = substitute(paste("Number of Candidate disks by ", epsilon) ~ temp_title, list(temp_title = temp_title))
g = ggplot(data=candidates, aes(x=factor(Dataset), y=Count, group = Disks, colour = Disks, shape = Disks)) + 
...
```{r Maximals2, echo=FALSE, fig.width=10.5, fig.height=7.5, fig.align='center'}
maximals = data[data$Disks == "Maximals",]
title = substitute(paste("Number of Maximal disks by ", epsilon) ~ temp_title, list(temp_title = temp_title))
g = ggplot(data=maximals, aes(x=factor(Dataset), y=Count, group = Disks, colour = Disks, shape = Disks)) + 
  geom_line(aes(linetype = Disks)) + 

...
filename ="Berlin_N20K-80K_E10.0-50.0_M12_P1024_201710290820"
...
data = anti_join(data, maxs, by = c("Dataset", "Epsilon", "Time"))
mins = sqldf("SELECT Dataset, Epsilon, Cores, min(Time) AS Time FROM data GROUP BY 1, 2, 3")
data = anti_join(data, mins, by = c("Dataset", "Epsilon", "Time"))

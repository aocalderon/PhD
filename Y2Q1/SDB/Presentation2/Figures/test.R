...
plot(x, y, asp=1, axes = F, xlim = c(0 - epsilon, n + epsilon), ylim = c(0 - epsilon, n + epsilon), xlab = "", ylab = "", pch = 21, cex = 0.8, col = 1, bg = 1 )
box()
...
for(i in 1:nrow(data)){
  ...
  if(d <= epsilon && d != 0){
    ...
    draw.circle(centers[1], centers[2], epsilon/2, nv = 1000, border = NULL, col = NA, lty = 2, lwd = 0.5)
    draw.circle(centers[3], centers[4], epsilon/2, nv = 1000, border = NULL, col = NA, lty = 2, lwd = 0.5)    
  }
}

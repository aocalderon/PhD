...
\begin{frame}{PBFE implementation}
    \begin{itemize}
     \item Written in Scala using Simba 1.6.0.
     \begin{itemize}
      \item \texttt{DISTANCE JOIN} and \texttt{CIRCLERANGE} operators were the key operators!!!
      \item Tested by visual inspection and counting number of found disks.
     \end{itemize}
     \item \large Demo (in R language):
     \begin{itemize}
...
\begin{frame}{Conclusions}
  \begin{itemize}\pause
   \item An implementation of a parallel method to detect disks for the BFE algorithm has been presented.\pause
   \item The method proves to be scalable and reliable.\pause
   ...
   \item DISTANCE `SELF' JOINS depict a better performance using DataFrame or RDD filters (avoid Spark-SQL).
  \end{itemize}
\end{frame}

\begin{frame}{Future work}
  \begin{itemize}\pause
   \item Work on a parallel strategy to prune redundant and `under $\mu$' disks.\pause
   \item Work on a parallel strategy to join the sets of valid disks between time intervals.\pause
   \item Explore more datasets to optimize Speedup and Scaleup values.\pause
   \item Data pre-processing and result visualization are still open issues.
  \end{itemize}
\end{frame}

...
object PBFE {
  ...
  def main(args: Array[String]) : Unit = {
   ...
   import sqlContext.implicits._
    ...
    val sample = args(2).toInt
    val tag = filename.substring(filename.lastIndexOf("/") + 1).split("\\.")(0).substring(1)

    val points = sc.textFile(filename).map(_.split(",")).map(p => PointItem(p(0).trim.toInt, p(1).trim.toDouble, p(2).trim.toDouble)).toDF()

    val p1 = points.limit(sample)
    ...
    println("PBFE," + epsilon + "," + sample + "," + 2*n + "," + (time2 - time1) / 1000.0)
    sc.stop()
  }
}

...
object PBFE {
  ...
  def calculateDisks(pair: Row) : Pair = {
    ...
    if (D2 == 0)
        null
    var root = math.pow(math.abs(4.0 * (r2 / D2) - 1.0), 0.5)
    var h1 = ((X + Y * root) / 2) + pair.getDouble(4)
    var h2 = ((X - Y * root) / 2) + pair.getDouble(4)
    var k1 = ((Y - X * root) / 2) + pair.getDouble(5)
    var k2 = ((Y + X * root) / 2) + pair.getDouble(5)
  ...
  def main(args: Array[String]) : Unit = {
    ...
    println("PBFE," + epsilon + "," + tag + "," + 2 * disks.count() + "," + (time2 - time1) / 1000.0)

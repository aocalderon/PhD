...
#define CACHE_BCOPY(cmd, blk, bofs, p, nbytes)	\
  if (cmd == Read)							\
    {									\
      switch (nbytes) {							\
      case 1:								\
	*((byte_t *)p) = CACHE_BYTE(&blk->data, bofs); break;	\
      case 2:								\
	*((half_t *)p) = CACHE_HALF(&blk->data, bofs); break;	\
      case 4:								\
	*((word_t *)p) = CACHE_WORD(&blk->data, bofs); break;	\
      default:								\
	{ /* >= 8, power of two, fits in block */			\
	  int words = nbytes >> 2;					\
	  while (words-- > 0)						\
	    {								\
	      *((word_t *)p) = CACHE_WORD(&blk->data, bofs);	\
	      p += 4; bofs += 4;					\
	    }\
	}\
      }\
    ...
    {									\
      switch (nbytes) {							\
      case 1:								\
	CACHE_BYTE(&blk->data, bofs) = *((byte_t *)p); break;	\
      case 2:								\
        CACHE_HALF(&blk->data, bofs) = *((half_t *)p); break;	\
      case 4:								\
	CACHE_WORD(&blk->data, bofs) = *((word_t *)p); break;	\
      default:								\
	{ /* >= 8, power of two, fits in block */			\
	  int words = nbytes >> 2;					\
	  while (words-- > 0)						\
	    {								\
	      CACHE_WORD(&blk->data, bofs) = *((word_t *)p);		\
	      p += 4; bofs += 4;					\
	    }\
	}\
    }\
  ...
  for (bindex=0,i=0; i<nsets; i++)
      ...
      if (cp->hsize)
	{
	 ...
	 during random replacement selection) */
      ...
      cp->sets[i].PLRU_state = (1 << width_PLRU_state) - 1;
...
cache_char2policy(char c)		/* replacement policy as a char */
{
  switch (c) {
  case 'l': return LRU;
  case 'p': return PLRU;  /*** CS203 Declare a case for PLRU ***/
	     ...
	     FILE *stream)		/* output stream */
{
  ...
  fprintf(stream,
	  ...
	  cp->name, cp->assoc,
	  cp->policy == LRU ? "LRU"
	  : cp->policy == PLRU? "PLRU" /*** CS203 Define PLRU as a policy option ***/
...
/*** CS203 PLRU algorithm ***/
int
get_PLRU_bindex ( int assoc,int PLRU_state ) 		/* current PLRU state */
{
	int bindex;
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case FIFO:
    ...
    break;
  case PLRU: /*** CS203 PLRU miss handler ***/
    {
...
unsigned int        /* latency of access in cycles */
dl1_cache_access(struct cache_t *cp,  /* cache to access */
       enum mem_cmd cmd,    /* access type, Read or Write */
       md_addr_t addr,    /* address of access */
       void *vp,      /* ptr to buffer for input/output */
       int nbytes,    /* number of bytes to access */
       ...
       md_addr_t *repl_addr)  /* for address of replaced block */
{
  ...
  *repl_addr = CACHE_MK_BADDR(cp, repl->tag, set);
  ...
  //   lat += cp->blk_access_fn(Write,
  //          CACHE_MK_BADDR(cp, repl->tag, set),
  //          cp->bsize, repl, now+lat, CACHE_MK_BADDR(cp, repl->tag, set)); // What's going on here??
  ...
  /* read data block */
  lat += cp->blk_access_fn(Read, CACHE_BADDR(cp, addr), cp->bsize,
         repl, now+lat, CACHE_MK_BADDR(cp, repl->tag, set));
...
unsigned int        /* latency of access in cycles */
dl1dsb_cache_access(struct cache_t *cp,  /* cache to access */
       enum mem_cmd cmd,    /* access type, Read or Write */
       md_addr_t addr,    /* address of access */
       void *vp,      /* ptr to buffer for input/output */
       int nbytes,    /* number of bytes to access */
...
unsigned int        /* latency of access in cycles */
il1isb_cache_access(struct cache_t *cp,  /* cache to access */
       enum mem_cmd cmd,    /* access type, Read or Write */
       md_addr_t addr,    /* address of access */
       void *vp,      /* ptr to buffer for input/output */
       int nbytes,    /* number of bytes to access */
...
/*** CS203 cache access function for victim cache ***/
unsigned int        /* latency of access in cycles */
vic_cache_access(struct cache_t *cp,  /* cache to access */
       enum mem_cmd cmd,    /* access type, Read or Write */
       md_addr_t addr,    /* address of access */
       void *vp,      /* ptr to buffer for input/output */
       int nbytes,    /* number of bytes to access */
       ...
       md_addr_t rep_addr)
{
  ...
  md_addr_t set = CACHE_SET(cp, addr);
  md_addr_t bofs = CACHE_BLK(cp, addr);
  md_addr_t rep_tag = CACHE_TAG(cp, rep_addr);
  struct cache_blk_t *blk, *repl;
  int lat = 0;
  ...
  case FIFO:
    repl = cp->sets[set].way_tail;
    //update_way_list(&cp->sets[set], repl, Head); // Updated after replacement.
    break;
  case Random:
    {
      int bindex = myrand() & (cp->assoc - 1);
  ...
  /* write back replaced block data, if it is dirty */
  if (repl->status & CACHE_BLK_VALID)
    {
      cp->replacements++;
  ...
  *repl_addr = CACHE_MK_BADDR(cp, repl->tag, set);
    ...
    if (repl->status & CACHE_BLK_DIRTY)
  {  //The VICTIM CACHE only writes back.
    /* write back the cache block */
    cp->writebacks++;
    lat += cp->blk_access_fn(Write,
           CACHE_MK_BADDR(cp, repl->tag, set),
  ...
  blk->tag = rep_tag;
  blk->user_data = rep_blk->user_data;
  blk->data = rep_blk->data;
  blk->ready = rep_blk->ready;
  blk->status = rep_blk->status;
  update_way_list(&cp->sets[set], blk, Head);
 ...
 cache_hit: /* slow hit handler */
  ...
  //if (cmd == Read) cp->read_hits++;
  byte_t rep_data = blk->data;
  byte_t rep_user_data = blk->user_data;
  if (blk->way_prev)
   {
      /* move this block to head of the way (MRU) list */
      update_way_list(&cp->sets[set], blk, Head);
...
//rep_tag is the reconstructed tag(pertaining to Victim cache structure) of the repl address from DL1.

  blk->tag = rep_tag;
  blk->user_data = rep_blk->user_data;
  blk->data = rep_blk->data;
  rep_blk->data = rep_data;
  rep_blk->user_data = rep_user_data; 
  blk->status = rep_blk->status;
...
/*** CS203 cache access function for data stream buffer ***/
unsigned int				/* latency of access in cycles */
dsbuf_cache_access(struct cache_t *cp,	/* cache to access */
	     enum mem_cmd cmd,		/* access type, Read or Write */
	     md_addr_t addr,		/* address of access */
	     void *vp,			/* ptr to buffer for input/output */
	     int nbytes,		/* number of bytes to access */
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  /* cs203A */
	for (fetch_counter = 1; fetch_counter <= 16; ++fetch_counter) {
...
/*** CS203 cache access function for instruction stream buffer ***/
unsigned int				/* latency of access in cycles */
isbuf_cache_access(struct cache_t *cp,	/* cache to access */
	     enum mem_cmd cmd,		/* access type, Read or Write */
	     md_addr_t addr,		/* address of access */
	     void *vp,			/* ptr to buffer for input/output */
	     int nbytes,		/* number of bytes to access */
	  ...
	  if (blk->status & CACHE_BLK_VALID)
	      ...
	      if (blk->status & CACHE_BLK_DIRTY)
		{
		  ...
		  lat += cp->blk_access_fn(Write,
					   ...
					   cp->bsize, blk, now+lat, CACHE_MK_BADDR(cp, blk->tag, i));
		}
	    }
	}
    }
  ...
  if (blk)
      ...
      if (blk->status & CACHE_BLK_DIRTY)
	{
	  ...
	  lat += cp->blk_access_fn(Write,
				   ...
				   cp->bsize, blk, now+lat, CACHE_MK_BADDR(cp, blk->tag, set));
	}

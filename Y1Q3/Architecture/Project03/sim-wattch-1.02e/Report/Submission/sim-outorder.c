...
/*** CS203 Declaring access variable ***/
...
/*** CS203 Go to victim cache if there is a DL1 miss ***/
...
/*** CS203 Go to data buffer if there is a DL1 miss ***/
...
/*** CS203  data buffer block miss handler function ***/
static unsigned int			/* latency of block access */
...
/*** CS203  instruction buffer block miss handler function ***/
static unsigned int			/* latency of block access */
...
/*** CS203 victim cache block miss handler function ***/
static unsigned int			/* latency of block access */
...
/*** CS203 go to instruction buffer if there is a miss in il1 cache ***/
if (isbuffer_cache)
    {
      /* access next level of data cache hierarchy */
...
/******************************************************************************************************/
/* CS203  Registering victim cache and stream buffers variables                                       */
/******************************************************************************************************/
...
opt_reg_string(odb, "-cache:isbuffer",
		 "Instruction Buffer cache config, i.e., {<config>|none}",
		 &isbuffer_cache_opt, /*FIXME!!!*/"isbuf:1:64:16:f",
		 /* print */TRUE, NULL);
...
/******************************************************************************************************/
	...
	else /* dl1 is defined */
	{
		...
		if (!mystricmp(cache_dl2_opt, "none"))
		{
			...
			/* the victim cache cannot be defined */
			if (strcmp(victim_cache_opt, "none"))
				fatal("the l2 data cache must defined if the victim cache is defined");
			victim_cache = NULL;
		}
		else /* dl2 is defined */
		{
...
/******************************************************************************************************/
/* CS203  Registering victim cache and stream buffers stats                                           */
/******************************************************************************************************/
  if (victim_cache)
		  ...
		  if (cache_dl1)
		    {
              ...
              /*** CS203 cache access functions for victim cache and data stream buffer ***/
			      ...
			      == (F_MEM|F_LOAD)))
			{
			  ...
			  if (!load_lat)
			      ...
			      if (cache_dl1 && valid_addr)
				{
				  ...
				  /*** CS203 cache access functions for victim and data stream buffer ***/
				  if(victim_cache){
				  load_lat =

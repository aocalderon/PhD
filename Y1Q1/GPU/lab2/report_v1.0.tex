...
$16\times16\times4\times30=30720$.
 ...
 The \textit{--ptxas-options=``-v''} option was included in the Makefile as shows figure \ref{fig:Makefile} (line 2).  The output result can be seen in figure \ref{fig:compiling}.  It shows the number of registers allocated (12), the size of share memory (around 2 Kb) and the number of bytes used for constant memory (12 bytes). It is consistent with the use of two floating-point arrays in share memory for matrices A and B, each of $16\times16$ (TILE\_SIZE). The remaining bytes and size of constant memory could be explained by the use of kernel arguments and internal operations.

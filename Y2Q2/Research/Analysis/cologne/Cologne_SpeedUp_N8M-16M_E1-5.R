...
data <- data.frame()
for(i in seq(0, 14)){
  filename <- paste0("../",DATASET,"1/", DATASET_TAG,"_PBFE_N8M-16M_E1-5_C",i,".csv")
...
for(i in seq(0, 14)){
  filename <- paste0("../",DATASET,"2/", DATASET_TAG,"_PBFE_N8M-16M_E1-5_C",i,".csv")
  temp <- read.csv(filename, header = F)
  temp <- temp[, c(2, 3, 5)]
  temp$Tag <- i
  data <- rbind(data, temp)
...
for(i in seq(0, 14)){
  filename <- paste0("../",DATASET,"3/", DATASET_TAG,"_PBFE_N8M-16M_E1-5_C",i,".csv")
  temp <- read.csv(filename, header = F)
  temp <- temp[, c(2, 3, 5)]
  temp$Tag <- i
  data <- rbind(data, temp)
...
core1 <- read.csv(paste0(DATASET_TAG,"_PBFE_LOCAL_N8M-16M_E1-5.csv"), header = F)
